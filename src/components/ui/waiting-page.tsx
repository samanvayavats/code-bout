...
export default function WaitingPage({
  ...
  const diffColor =
    difficulty === "easy"   ? "text-[#2ECC71] bg-[#2ECC71]/10" :
    difficulty === "medium" ? "text-[#F4D03F] bg-[#F4D03F]/10" :
    ...
    <main className="bg-[#0A0A0F] text-[#F0EFF4] min-h-screen flex flex-col items-center justify-center px-6 font-sans">
      ...
      <div className="w-full max-w-md border border-[#1E1E2E] rounded-2xl bg-[#111118] overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#1E1E2E] bg-[#E63946]/5">
          ...
          <span className={`font-mono text-[10px] font-semibold px-2 py-1 rounded-md uppercase tracking-wide ${diffColor}`}>
            {difficulty}
          </span>
        </div>
        ...
        <div className="p-8 flex flex-col items-center text-center">
            ...
            <>
              ...
              <div className="relative mb-8">
                <svg width="130" height="130" className="-rotate-90">
                  ...
                  <circle
                    cx="65" cy="65" r={radius}
                    fill="none" stroke="#E63946" strokeWidth="6"
                    strokeLinecap="round"
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center rotate-0">
                  ...
                  <span className="font-mono text-[10px] text-[#6B6B80] uppercase tracking-widest">sec</span>
                </div>
              </div>
          ...
          {status === "matched" && (
            <>
              ...
              {/* problem */}
              <div className="w-full bg-[#0A0A0F] border border-[#1E1E2E] rounded-xl p-4 text-left mb-6">
                <p className="font-mono text-[10px] uppercase tracking-widest text-[#6B6B80] mb-2">Problem</p>
                <p className="text-sm font-semibold">{problemTitle}</p>
              </div>
              ...
              <div className="w-full h-1 bg-[#1E1E2E] rounded-full mt-8 overflow-hidden">
                <div className="h-full bg-[#E63946] rounded-full animate-[loading_2s_ease-in-out_forwards]"
                  style={{ animation: "width 2s linear forwards", width: "100%" }}
                />
              </div>
              <p className="font-mono text-[10px] text-[#6B6B80] mt-2">Redirecting to battlefield{dots}</p>
            </>
          )}
      ...
      {status === "searching" && (
        <p className="mt-6 font-mono text-[11px] text-[#6B6B80] text-center max-w-sm">
          Matching you with someone on the same problem and difficulty.
          If no match in 60s, criteria will relax.
        </p>
      )}
